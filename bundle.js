...
const Books = (() => {
  ...
  async function searchGoogleBooks(query) {
    ...
    // API key olmasa da çalışır, sadece kota daha düşük
    const keyParam = (key && key !== 'YOUR_GOOGLE_BOOKS_API_KEY') ? `&key=${key}` : '';
    const url = `${base}?q=${query}&maxResults=5${keyParam}`;
    const res = await fetch(url);
    ...
    return json.items.map(item => {
      ...
      // Kapak için HTTPS versiyonunu tercih et
      let cover = '';
      if (info.imageLinks) {
        cover = info.imageLinks.extraLarge || info.imageLinks.large ||
